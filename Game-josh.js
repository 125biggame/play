...
window.addEventListener("load", function () {
    ...
    class Fan {
        ...
        draw(context, color){
            ...
            if (color == "red"){
                this.image = document.getElementById("treeFan");
            }
            else{
                this.image = document.getElementById("bearFan");
            }
            context.drawImage(this.image, this.x, this.y, this.width, this.height);
        }
    ...
    for(let i = 0; i < 5; i++){
        for(let j = 0; j < 10; j++){
            const fan = new Fan(canvas.width, canvas.height, canvas.width * (j + 1) / 11, canvas.height * (i + 1) / 11);
            fans.push(fan);
        }
    }
